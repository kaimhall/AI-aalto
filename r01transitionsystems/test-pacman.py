...
breadthFirstSearch(
    ISTATE1, lambda state: (state.x == 0) and (state.y == 0) and (state.d != "N")
)

# Another grid
GRID2 = PacManGrid(
    [".......", ".XXX.X.", ".XXX...", ".XXX.X.", ".....X.", ".XXXXX.", "......."]
)
...
# Back to 0,0 but not facing North
breadthFirstSearch(
    ISTATE2, lambda state: (state.x == 0) and (state.y == 0) and (state.d != "N")
)
